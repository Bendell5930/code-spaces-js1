...
export function checkIn(venue, status, machineName) {
  ...
  // Auto-complete the "Check in at a venue" bingo task
  try { autoCompleteBingoTask('b6', 'check-in') } catch { /* ignore */ }
  return active
}
...
export function recordDailyCheckIn() {
  ...
  // Auto-complete "Maintain a 3-day streak" bingo task once reached
  if (data.currentStreak >= 3) {
    try { autoCompleteBingoTask('b18', 'streak') } catch { /* ignore */ }
  }
...
export function postReview(venueName, venueSuburb, ratings, comment) {
  ...
  // Auto-complete "Rate a venue" bingo task
  try { autoCompleteBingoTask('b19', 'review') } catch { /* ignore */ }
...
export function postHotTip(venueName, machineName, tipText) {
  ...
  // Auto-complete "Post a hot tip" bingo task
  try { autoCompleteBingoTask('b12', 'hot-tip') } catch { /* ignore */ }
...
const BINGO_PROGRESS_KEY = 'pokie-bingo-progress'
const SESSION_GAP_MS = 30 * 60 * 1000 // 30 min idle = new session

function _emptyBingoProgress() {
  return {
    brandsEver: [],         // lowercase brand strings ever played
    spinsByDay: {},         // { 'YYYY-MM-DD': count }
    spinsByWeek: {},        // { '<weekStartMs>': count }
    venuesByDay: {},        // { 'YYYY-MM-DD': [venueName...] }
    venuesByWeek: {},       // { '<weekStartMs>': [venueName...] }
    sessionStartTs: 0,
    sessionBonusCount: 0,
    sessionFirstWinTs: 0,
    lastSpinTs: 0,
...
function loadBingoProgress() {
  return load(BINGO_PROGRESS_KEY, _emptyBingoProgress())
}

function saveBingoProgress(p) { save(BINGO_PROGRESS_KEY, p) }
...
function _brandFrom(machineName) {
  if (!machineName) return null
  // Brand = part before " — " / " - " separator (e.g. "Dragon Link — Panda Magic" → "Dragon Link")
  return machineName.split(/\s+[—-]\s+/)[0].trim()
}

function _emitBingoUpdate(detail) {
  if (typeof window === 'undefined') return
  try { window.dispatchEvent(new CustomEvent('pokie-bingo-update', { detail })) } catch { /* ignore */ }
}

// Friendly labels surfaced in toasts when a task auto-completes
const BINGO_AUTO_LABELS = {
  b1: 'Bonus on a Monday',
  b2: 'Played 3 different brands',
  b4: 'Won $50+ in one spin',
  b5: '20 spins in a day',
  b6: 'Checked in at a venue',
  b7: 'Played Dragon Link',
  b8: 'Played Lightning Link',
  b9: '3 bonuses in a session',
  b10: 'Visited 2 venues in a day',
  b11: 'Won more than you bet',
  b12: 'Posted a hot tip',
  b13: 'Spin after 6pm',
  b16: 'Spins at 3 venues this week',
  b17: 'Bonus on a weekend',
  b18: '3-day streak',
  b19: 'Rated a venue',
  b21: '50 spins this week',
  b22: 'Win on the first spin',
  b23: 'Tried a new brand',
}

/**
 * Mark a single bingo task as complete via auto-detection.
 * Records `autoCompleted` metadata so the UI can highlight AI-filled cells.
 * Emits a `pokie-bingo-update` window event so the card re-renders live.
 * Returns null when the task is unknown or already complete.
 */
export function autoCompleteBingoTask(taskId, source = 'auto') {
  if (!taskId) return null
  const card = loadBingoCard()
  const cell = card.grid.find(g => g.id === taskId)
  if (!cell || cell.completed) return null
  cell.completed = true
  cell.autoCompleted = true
  cell.autoSource = source
  cell.autoTimestamp = Date.now()
  save(BINGO_KEY, card)
  const label = BINGO_AUTO_LABELS[taskId] || cell.text
  _emitBingoUpdate({ taskId, source, label })
  return { card, taskId, label, source }
}

/**
 * Returns quantitative progress towards multi-step bingo tasks so the UI
 * can show "8 / 20" style hints on cells that aren't ticked yet.
 */
export function getBingoTaskProgress() {
  const p = loadBingoProgress()
  const today = _ymd(Date.now())
  const week = String(_weekStartMs(Date.now()))
  return {
    b2: { current: Math.min(3, (p.brandsEver || []).length), target: 3 },
    b5: { current: Math.min(20, p.spinsByDay?.[today] || 0), target: 20 },
    b9: { current: Math.min(3, p.sessionBonusCount || 0), target: 3 },
    b10: { current: Math.min(2, (p.venuesByDay?.[today] || []).length), target: 2 },
    b16: { current: Math.min(3, (p.venuesByWeek?.[week] || []).length), target: 3 },
    b21: { current: Math.min(50, p.spinsByWeek?.[week] || 0), target: 50 },
  }
}

/**
 * Update auto-detection state from a spin event and auto-complete any
 * bingo tasks whose conditions are now met. Designed to be called from
 * the main app's `handleSubmit` for every spin / win / bonus event
 * recorded by the AI live scan or the manual calculator.
 *
 * @param {Object} spin  { machineName, betAmount, winAmount, bonusHit, timestamp, source, location, venueName }
 * @param {Object} ctx   { activeVenueName, sessionTotalBet, sessionTotalWon }
 * @returns {Array<{taskId,label,source}>} tasks that were just auto-completed
 */
export function evaluateBingoFromSpin(spin = {}, ctx = {}) {
  const ts = spin.timestamp || Date.now()
  const date = new Date(ts)
  const day = date.getDay()      // 0=Sun .. 6=Sat
  const hour = date.getHours()
  const today = _ymd(ts)
  const week = String(_weekStartMs(ts))
  const venueName = ctx.activeVenueName || spin.venueName || spin.location || null

  const p = loadBingoProgress()

  // ── Session boundary detection (>30 min idle = new session) ──
  const isNewSession = !p.lastSpinTs || (ts - p.lastSpinTs) > SESSION_GAP_MS
  if (isNewSession) {
    p.sessionStartTs = ts
    p.sessionBonusCount = 0
    p.sessionFirstWinTs = 0
  }

  // ── Brand tracking ──
  const brand = _brandFrom(spin.machineName)
  let isNewBrand = false
  if (brand) {
    p.brandsEver = p.brandsEver || []
    const lower = brand.toLowerCase()
    if (!p.brandsEver.includes(lower)) {
      p.brandsEver.push(lower)
      isNewBrand = true
    }
  }

  // ── Spin counters (only count actual wager events to avoid
  //    double-counting win-only / bonus-only follow-up records) ──
  p.spinsByDay = p.spinsByDay || {}
  p.spinsByWeek = p.spinsByWeek || {}
  const isWagerEvent = (spin.betAmount || 0) > 0
  if (isWagerEvent) {
    p.spinsByDay[today] = (p.spinsByDay[today] || 0) + 1
    p.spinsByWeek[week] = (p.spinsByWeek[week] || 0) + 1
  }

  // ── Venue tracking ──
  p.venuesByDay = p.venuesByDay || {}
  p.venuesByWeek = p.venuesByWeek || {}
  if (venueName) {
    if (!p.venuesByDay[today]) p.venuesByDay[today] = []
    if (!p.venuesByDay[today].includes(venueName)) p.venuesByDay[today].push(venueName)
    if (!p.venuesByWeek[week]) p.venuesByWeek[week] = []
    if (!p.venuesByWeek[week].includes(venueName)) p.venuesByWeek[week].push(venueName)
  }

  // ── Session bonuses & first-spin win ──
  if (spin.bonusHit) p.sessionBonusCount = (p.sessionBonusCount || 0) + 1
  const isFirstSpinOfSession = isNewSession && isWagerEvent
  if (isFirstSpinOfSession && (spin.winAmount || 0) > 0) {
    p.sessionFirstWinTs = ts
  }

  p.lastSpinTs = ts

  // ── Garbage-collect old daily / weekly buckets ──
  const cutoffDay = new Date(ts); cutoffDay.setDate(cutoffDay.getDate() - 14)
  const cutoffDayStr = _ymd(cutoffDay.getTime())
  for (const k of Object.keys(p.spinsByDay)) if (k < cutoffDayStr) delete p.spinsByDay[k]
  for (const k of Object.keys(p.venuesByDay)) if (k < cutoffDayStr) delete p.venuesByDay[k]
  const cutoffWeek = Number(week) - 8 * 7 * 86400 * 1000
  for (const k of Object.keys(p.spinsByWeek)) if (Number(k) < cutoffWeek) delete p.spinsByWeek[k]
  for (const k of Object.keys(p.venuesByWeek)) if (Number(k) < cutoffWeek) delete p.venuesByWeek[k]

  saveBingoProgress(p)

  // ── Evaluate task conditions ──
  const machineLower = (spin.machineName || '').toLowerCase()
  const matched = []
  if (spin.bonusHit && day === 1) matched.push('b1')
  if (p.brandsEver.length >= 3) matched.push('b2')
  if ((spin.winAmount || 0) >= 50) matched.push('b4')
  if ((p.spinsByDay[today] || 0) >= 20) matched.push('b5')
  if (machineLower.includes('dragon link')) matched.push('b7')
  if (machineLower.includes('lightning link')) matched.push('b8')
  if ((p.sessionBonusCount || 0) >= 3) matched.push('b9')
  if ((p.venuesByDay[today] || []).length >= 2) matched.push('b10')
  if ((ctx.sessionTotalWon || 0) > (ctx.sessionTotalBet || 0) && (ctx.sessionTotalBet || 0) > 0) matched.push('b11')
  if (hour >= 18) matched.push('b13')
  if ((p.venuesByWeek[week] || []).length >= 3) matched.push('b16')
  if (spin.bonusHit && (day === 0 || day === 6)) matched.push('b17')
  if ((p.spinsByWeek[week] || 0) >= 50) matched.push('b21')
  if (isFirstSpinOfSession && (spin.winAmount || 0) > 0) matched.push('b22')
  if (isNewBrand) matched.push('b23')

  const source = spin.source || 'spin'
  const completed = []
  for (const taskId of matched) {
    const r = autoCompleteBingoTask(taskId, source)
